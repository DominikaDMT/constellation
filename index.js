class Sky {
  ...
  // gwaizdka:
  // x: 100, y:100,, color: 'white', radius:10;
  ...
  drawStar(star) {
    ...
    this.ctx.fill();
    // przywrócenie stany cancasa
    this.ctx.restore();
  }

  // czyszczenie canvas, rysowanie gwiazd, animowanie i aktualizowanie
  draw(now) {
    this.delta = now - this.lastUpdate;
    // console.log(now);
    // 60 klatek na sekundę - standardowo
    this.clearCanvas();
    this.updateStars();
    this.drawStars();
  ...
  // metoda odpowiedzialna za inicjalizację i uruchomienie animacji
  run() {
    this.initCanvas();
